...
public class RedisUtils implements CommandLineRunner {
	...
	private void init() {
		...
		pool = new JedisPool(config, address, port, timeout, password, database);
		try {
			pool.getResource();
			logger.info("==> Redis server connected. Server address: " + address + ":" + port + "#" + database);
	...
	/**
	 * <p>
	 * 向redis存入key、value和nxxx,并释放连接资源
	 * </p>
	 * <p>
	 * 如果key已经存在 则覆盖
	 * </p>
	 ...
	 * @param ttl
	 *            超时时间
	 * @return 1: 设置成功 0：key不存在
	 */
	public static long expire(String key, int ttl) {
		Jedis jedis = null;
		try {
			jedis = pool.getResource();
	 ...
	 * @return
	 */
	public static Long rpush(String key, int time, String... strs) {
		Jedis jedis = null;
		Long res = null;
		try {
			jedis = pool.getResource();
